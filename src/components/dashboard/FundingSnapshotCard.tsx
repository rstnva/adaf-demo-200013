...
export function FundingSnapshotCard() {
  ...
  const hasData = Array.isArray(fundingData) && fundingData.length > 0;
  ...
  return (
    <Card className="adaf-card adaf-hover-lift">
      <CardHeader className="adaf-card-header">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            Funding Radar
            <ConceptInfo concept="Funding Radar" />
          </CardTitle>
      ...
      <CardContent className="adaf-card-content">
        {!hasData ? (
          <div className="text-center text-red-500 py-8">No funding data available.</div>
        ) : (
          <>
            ...
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-yellow-800">
                  {fundingData.filter(f => f.rate < 0).length} exchanges showing negative funding 48h+
                </span>
              </div>
            </div>
          </>
