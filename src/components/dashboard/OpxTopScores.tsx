...
export function OpxTopScores() {
  ...
  const hasData = Array.isArray(opportunities) && opportunities.length > 0;
      ...
      <CardHeader className="adaf-card-header">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            Mejores Oportunidades OP-X
            <ConceptInfo concept="OP-X Top Scores" />
          </CardTitle>
          <Link href="/opx">
        ...
        <div className="text-sm text-gray-600">
          Clasificadas por score compuesto de oportunidad
        </div>
      </CardHeader>
      <CardContent className="adaf-card-content">
        {!hasData ? (
          <div className="text-center text-red-500 py-8">No opportunity data available.</div>
        ) : (
          <>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">Return Est.</span>
                      </div>
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="text-sm text-blue-800">
                <strong>{opportunities.length}</strong> oportunidades arriba del umbral de 75 puntos. 
                <strong>{opportunities.filter(o => o.status === 'blocking').length}</strong> bloqueada(s) pendiente de revisión de riesgo.
              </div>
            </div>
          </>
        )}
